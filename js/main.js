...
//        },
//    });
    $(".accordion-unit").find(".accordion-title").on('click', function(e){
        e.preventDefault();
        $(this).toggleClass('active').siblings('.accordion-desc').slideToggle();
    })
    $(".mos-popup-activator").click(function(e){
        e.preventDefault();
        $("#videoPopupModal").find('iframe').attr('src', $(this).data('url'));
        $("#videoPopupModal").modal('show');
    });
    $("#videoPopupModal").on("hidden.bs.modal", function () {
        $(this).find('iframe').attr('src', '');
    });
    $('.mos-video-activator').on('click', function(ev) { 
        ev.preventDefault(); 
        $(this).closest('.mos-video-wrapper').addClass('video-activated');
        if ($(this).data('type') == 'iframe') {
//            $(this).closest('.mos-video-wrapper').find('iframe')[0].src += $(this).data('url') + "&autoplay=1";
            $(this).closest('.mos-video-wrapper').find('iframe').attr('src', $(this).data('url') + "&autoplay=1");
        } else {
            $(this).closest('.mos-video-wrapper').find('video').attr('src', $(this).data('url'));
        }
    });
